...
//       assert( vertices_.size() >0 );
    ...
    void write_to_file(const std::string& filename,
		       int fios_precision_temp=FIOS_PRECISION) const;
    //Mutators
    /** \brief  access with automatic wrap-around in forward direction
     *
     * \remarks  for efficiency, no bounds check; usually trying to
...
//       assert( vertices_.size() );
    ...
    void clear()
    { vertices_.clear(); }
    /// preallocate some space for subsequential push_back() operations.
    void reserve(std::vector<Point>::size_type n)
    { vertices_.reserve(n); }
